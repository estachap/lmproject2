...
abstract class BasePropelcommit extends BaseObject implements Persistent
{
    ...
    {
        $pos = PropelcommitPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
    ...
    {
        if (isset($alreadyDumpedObjects['Propelcommit'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Propelcommit'][$this->getPrimaryKey()] = true;
        $keys = PropelcommitPeer::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
