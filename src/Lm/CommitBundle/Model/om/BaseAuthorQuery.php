 ...
 * @method AuthorQuery orderByUri($order = Criteria::ASC) Order by the uri column
 *
 * @method AuthorQuery groupById() Group by the id column
 * @method AuthorQuery groupByName() Group by the name column
 * @method AuthorQuery groupByUri() Group by the uri column
 *
 ...
 *
 * @method AuthorQuery leftJoinPropelcommit($relationAlias = null) Adds a LEFT JOIN clause to the query using the Propelcommit relation
 * @method AuthorQuery rightJoinPropelcommit($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Propelcommit relation
 * @method AuthorQuery innerJoinPropelcommit($relationAlias = null) Adds a INNER JOIN clause to the query using the Propelcommit relation
 *
 ...
 * @method Author findOneByUri(string $uri) Return the first Author filtered by the uri column
 *
 ...
 * @method array findByUri(string $uri) Return Author objects filtered by the uri column
 */
abstract class BaseAuthorQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `id`, `name`, `uri` FROM `author` WHERE `id` = :p0';
        try {
    ...
    public function filterByUri($uri = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(AuthorPeer::URI, $uri, $comparison);
    }

    /**
     * Filter the query by a related Propelcommit object
     *
     * @param   Propelcommit|PropelObjectCollection $propelcommit  the related object to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 AuthorQuery The current query, for fluid interface
    ...
    public function filterByPropelcommit($propelcommit, $comparison = null)
    {
        if ($propelcommit instanceof Propelcommit) {
            return $this
                ->addUsingAlias(AuthorPeer::ID, $propelcommit->getAuthorId(), $comparison);
        } elseif ($propelcommit instanceof PropelObjectCollection) {
            return $this
                ->usePropelcommitQuery()
                ->filterByPrimaryKeys($propelcommit->getPrimaryKeys())
                ->endUse();
        } else {
